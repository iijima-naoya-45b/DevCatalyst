...
export function OAuthButton({ provider, className }: OAuthButtonProps) {
    ...
    const handleClick = () => {
        // developerプロバイダーは開発環境専用のため、実際のOAuth認証は行わない
    ...
    const getProviderStyles = () => {
        switch (provider) {
            case 'google':
                return {
                    bg: 'bg-white hover:bg-gray-50 dark:bg-slate-700 dark:hover:bg-slate-600 border border-gray-300 dark:border-gray-600',
    ...
    return (
        <Button
            onClick={handleClick}
            className={`w-full flex items-center justify-center gap-3 py-3 px-4 rounded-lg font-medium transition-all duration-200 transform hover:scale-[1.02] ${styles.bg} ${styles.text} ${className}`}
            variant="outline"
        >
            {styles.icon}
