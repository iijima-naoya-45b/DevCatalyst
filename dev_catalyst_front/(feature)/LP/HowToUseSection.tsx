...
// ビジュアライゼーション
const ProductEvolution = dynamic(
  () =>
    import('../../components/ui/visualizations/ProductEvolution').then((mod) => ({
      default: mod.ProductEvolution,
    })),
  {
    ssr: false,
  }
...
const personas = [
  {
    ...
    ariaDialogue: [
      ...
      {
        role: 'aria',
        text: 'まず市場を見てみましょう。既存の予約システムは月額1-3万円が相場ですね。差別化ポイントは「小規模店舗向けの低価格」と「LINE連携」が有効です。',
      },
      { role: 'user', text: 'なるほど。技術的にはどう作れば？' },
      {
        role: 'aria',
        text: 'Next.js + Supabaseで構築すれば、月額コストは¥2,000以下。LINEミニアプリとの連携も可能です。まずは予約機能とカレンダー表示から始めましょう。',
      },
    ],
    vision: {
      title: '目指すSaaSプロジェクト',
  ...
  },
  {
    ...
    ariaDialogue: [
      {
        role: 'user',
        text: '週末だけでSaaSを作りたいです。エンジニア向けのドキュメント管理ツールを考えています。',
      },
      {
        role: 'aria',
        text: 'Notion、Confluenceとの差別化が重要です。エンジニア向けなら「コードスニペット管理」「API自動ドキュメント生成」が有効です。',
      },
      { role: 'user', text: '技術スタックは？' },
      {
        role: 'aria',
        text: 'Next.js + MDX + Prisma + PostgreSQLがおすすめです。週末開発なら、まずMarkdownエディタとバージョン管理から。GitHubログインで即使えるようにしましょう。',
      },
    ],
    vision: {
      title: '目指すSaaSプロジェクト',
  ...
  },
  {
    ...
    ariaDialogue: [
      {
        role: 'user',
        text: 'デザイナー向けのアセット管理ツールを作りたいです。でもビジネスモデルが...',
      },
      {
        role: 'aria',
        text: 'デザイナーの課題は「アセットの散在」と「バージョン管理」ですね。サブスクモデルで、個人¥980、チーム¥4,980が適正価格です。',
      },
      { role: 'user', text: 'どんな機能が必要ですか？' },
      {
        role: 'aria',
        text: 'クラウドストレージ、AIタグ付け、Figma/Adobe連携、そしてデザインシステム管理。最初はクラウドストレージ+タグ検索から始めましょう。',
      },
    ],
    vision: {
      title: '目指すSaaSプロジェクト',
  ...
  },
  {
    ...
    ariaDialogue: [
      {
        role: 'user',
        text: '製造業のお客様に、在庫管理システムの提案をしたいです。競合他社の分析と、お客様の課題に合わせた提案内容を考えてほしいです。',
      },
      {
        role: 'aria',
        text: '製造業の在庫管理システム市場を分析しました。主要競合はA社（月額¥50,000）、B社（月額¥80,000）です。差別化ポイントは「中小製造業向けの低価格」と「既存ERP連携」が有効です。',
      },
      {
        role: 'user',
        text: 'お客様は「在庫の見える化」と「発注の自動化」を課題にしています。提案書の構成を考えてください。',
      },
      {
        role: 'aria',
        text: '提案書の構成案：1. 現状課題の整理（在庫の見える化不足、手動発注の非効率）、2. 解決策（リアルタイム在庫ダッシュボード、AI発注予測）、3. 導入効果（作業時間30%削減、在庫コスト15%削減）、4. 導入スケジュール。Spec生成機能で詳細な提案書を作成できます。',
      },
    ],
    vision: {
      title: '営業活動の効率化',
  ...
  return (
    <section className="py-16 sm:py-20 px-3 sm:px-4 bg-gradient-to-b from-white to-gray-50 dark:from-slate-950 dark:to-slate-900 overflow-x-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div
          ref={headerRef}
          className={`text-center mb-12 sm:mb-16 fade-in-up ${headerInView ? 'in-view' : ''}`}
        >
          <Badge className="bg-gold/10 gold-soft-text border-gold/30 mb-4">HOW TO USE</Badge>
          ...
          <p className="text-sm sm:text-base md:text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            ...
            <span className="gold-soft-text font-semibold">
              行動経済学・心理学・マーケティングの視点
            </span>
            から、あなたの心理体験に基づいて提案します。
          </p>
        </div>
        ...
        <div
          ref={differenceRef}
          className={`mb-16 sm:mb-20 fade-in-up ${differenceInView ? 'in-view' : ''}`}
        >
            ...
            <CardContent className="p-6 sm:p-8 md:p-10">
              <div className="text-center mb-6">
                ...
                <p className="text-sm sm:text-base text-gray-700 dark:text-gray-300 max-w-3xl mx-auto">
                  ...
                  <span className="gold-soft-text font-semibold">RAG技術による戦略特化</span>
                  により、
        ...
        <div ref={personaRef}>
          <h3
            className={`text-xl sm:text-2xl font-bold text-center mb-8 sm:mb-12 text-gray-900 dark:text-white fade-in-up ${personaInView ? 'in-view' : ''}`}
          >
          ...
          {/* ペルソナタブ */}
          <div
            className={`flex justify-center gap-3 sm:gap-4 mb-8 flex-wrap fade-in-up ${personaInView ? 'in-view delay-200' : ''}`}
          >
            {personas.map((persona) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`w-4 h-4 sm:w-5 sm:h-5 ${selectedPersona.id === persona.id ? 'text-aria-dark-soft' : 'gold-soft-text'}`}
                  />
                  {persona.label}
                </button>
              );
          ...
          {/* ペルソナストーリー */}
          <Card
            className={`max-w-5xl mx-auto border-gold/30 bg-white dark:bg-slate-800 shadow-xl scale-in ${personaInView ? 'in-view delay-300' : ''}`}
          >
              ...
              <div className="space-y-6 sm:space-y-8">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <Badge
                      variant="outline"
                      className="ml-auto border-gold/30 gold-soft-text text-xs"
                    >
                      心理体験に基づく提案
                    </Badge>
                  </div>
                ...
                <div className="p-4 sm:p-6 aria-gold-surface rounded-xl border-2 border-gold/35">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h5 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white">
                      {selectedPersona.vision.title}
                    </h5>
                  </div>

                  <p className="text-xl sm:text-2xl font-bold gold-soft-text mb-4 sm:mb-6">
                    {selectedPersona.vision.description}
                  </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 p-3 sm:p-4 bg-white/50 dark:bg-slate-900/50 rounded-lg">
                    <div>
                      ...
                      <p className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-white">
                        {selectedPersona.vision.metrics.target}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">目標売上</p>
                      <p className="font-semibold text-xs sm:text-sm gold-soft-text">
                        {selectedPersona.vision.metrics.revenue}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">スケジュール</p>
                      <p className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-white">
                        {selectedPersona.vision.metrics.timeline}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-6">
                    <div>
                      ...
                      <p className="font-bold text-sm sm:text-base text-gray-900 dark:text-white">
                        {selectedPersona.result.period}
                      </p>
                    </div>
                    <div className="flex-1">
                      <p className="text-xs text-gray-600 dark:text-gray-400">達成</p>
                      <p className="font-bold text-sm sm:text-base gold-soft-text">
                        {selectedPersona.result.achievement}
                      </p>
                    </div>
                  </div>
                </div>
