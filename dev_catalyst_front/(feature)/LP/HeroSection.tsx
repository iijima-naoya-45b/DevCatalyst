    ...
    <section className="relative min-h-screen flex items-center justify-center px-3 sm:px-4 py-12 sm:py-16 md:py-20 bg-gradient-to-br from-gradient-start via-gradient-mid to-gradient-end dark:bg-gradient-to-br dark:from-slate-900/80 dark:via-slate-950 dark:to-black">
      ...
      <div className="max-w-7xl mx-auto text-center relative z-10">
        ...
        <div className="flex items-center justify-center space-x-3 sm:space-x-4 mb-6 sm:mb-8 md:mb-10">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-serif font-bold bg-gradient-to-r from-amber-500 via-yellow-500 to-orange-500 bg-clip-text text-transparent dark:from-gold dark:via-gold-light dark:to-gold dark:text-gold subtle-glow">
              devCatalyst
            </h1>
            <p className="text-gold-enhanced dark:text-gold/90 text-xs sm:text-sm tracking-wide mt-1">AI Strategy Partner "Vertex"</p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto mb-16 sm:mb-24 md:mb-32 lg:mb-36">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl font-serif font-bold mb-8 sm:mb-12 md:mb-16 lg:mb-20 xl:mb-24 leading-tight sm:leading-relaxed px-2">
            <span className="text-slate-800 drop-shadow-[0_1px_2px_rgba(255,255,255,0.5)] dark:text-yellow-400 dark:drop-shadow-[0_2px_4px_rgba(255,255,255,0.8)] transition-all duration-300">
              深夜、戦略に迷うあなたへ。<br />
              迷いを戦略に変える、AI戦略パートナー。
            </span>
          </h2>
          ...
          <div className="space-y-8 sm:space-y-12 md:space-y-16 lg:space-y-20">
            <p className="text-sm sm:text-base md:text-lg lg:text-xl text-slate-700 dark:text-gray-300 leading-relaxed font-light max-w-5xl mx-auto px-3 sm:px-4 md:px-0 py-2 sm:py-4">
              営業戦略、事業ピボット、新規プロダクト…。<br />
              孤独な夜の問いを、AIが本質から整理し、明日への一手へとつなげる。<br /><br />
              AI戦略解析 <strong className="text-gold">devCatalist〈Vertex〉</strong> は、<br />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 md:gap-12 lg:gap-16 max-w-5xl mx-auto mt-12 sm:mt-16 md:mt-20 lg:mt-24 px-3 sm:px-4 md:px-0">
              {heroCheckpoints.map((checkpoint, index) => (
                <div key={index} className="flex items-start space-x-3 sm:space-x-4 md:space-x-6 text-left py-4 sm:py-6 px-2 sm:px-4">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 bg-gold/10 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                  ...
                  <div className="space-y-2 sm:space-y-3 md:space-y-4">
                    ...
                    <p className="text-sm sm:text-base md:text-lg text-slate-600 dark:text-gray-400 leading-relaxed">{checkpoint.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {heroStats.map((stat, index) => {
            ...
            return (
              <div key={index} className="text-center p-3 sm:p-4 md:p-6 lg:p-8 xl:p-10 space-y-2 sm:space-y-3 md:space-y-4 lg:space-y-5">
                <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 lg:w-18 lg:h-18 bg-gold/10 rounded-full flex items-center justify-center mx-auto">
                ...
                <div className="text-xs sm:text-sm md:text-base lg:text-lg text-slate-600 dark:text-gray-400 leading-relaxed max-w-[140px] sm:max-w-[160px] md:max-w-[180px] lg:max-w-[200px] mx-auto">{stat.label}</div>
              </div>
            );
          })}
        </div>
