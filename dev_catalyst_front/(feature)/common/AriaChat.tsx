...
import { Send, Sparkles, TrendingUp, Target, Lightbulb, Brain, Zap, BarChart3 } from 'lucide-react';
import {
  Chart as ChartJS,
  ArcElement,
  CategoryScale,
...
export function AriaChat({ onStartAnalysis, showUserAvatar = true }: AriaChatProps) {
    ...
    setTimeout(
      () => {
        ...
        if (userInput.includes('競合') || userInput.includes('ライバル')) {
            ...
            [
              {
                ...
                title: 'ブルーオーシャン戦略',
                description:
                  '競合のいない市場を見つける：例）飲食店→特定の食事制限のある人向けに特化',
              },
              {
                type: 'opportunity',
                title: 'ニッチ市場の可能性',
          ...
          addAriaMessage(
            ...
            [
              ...
              },
              {
                ...
                title: '3ヶ月集中戦略',
                description:
                  '1つの強みに絞り、3ヶ月間徹底的に磨く。複数の中途半端より、1つの確実な強み',
              },
            ]
          );
          ...
          addAriaMessage(
            ...
            [
              ...
              },
              {
                ...
                title: '市場調査の実践法',
                description:
                  '①Google Trends ②SNS検索 ③競合のレビュー分析 で、1週間で基礎調査が完了',
              },
              {
                type: 'risk',
                title: 'タイミングの重要性',
          ...
          userInput.includes('価格') ||
          userInput.includes('料金') ||
          userInput.includes('値段')
        ) {
          addAriaMessage(
            ...
            [
              {
                ...
                title: '段階的価格設定',
                description:
                  '3つのプラン（ベーシック・スタンダード・プレミアム）で選択肢を提供。中間が最も選ばれやすい',
              },
              {
                type: 'opportunity',
                title: '心理的価格戦略',
                description:
                  '2,980円と3,000円で印象が大きく変わる。端数価格の活用で購入率が20%向上することも',
              },
              {
                type: 'risk',
                title: '安売りの危険性',
                description:
                  '最初から安くすると、後で値上げが困難に。「高品質・高価格」で始める方が長期的に有利',
              },
            ]
          );
        } else if (
          userInput.includes('マーケティング') ||
          userInput.includes('集客') ||
          userInput.includes('広告')
        ) {
          addAriaMessage(
            ...
            [
              {
                ...
                title: '1点集中戦略',
                description:
                  '最初の3ヶ月は1つのチャネルに集中。複数の中途半端より、1つの確実な流入源を',
              },
              {
                type: 'opportunity',
                title: 'コミュニティマーケティング',
              ...
              },
              {
                ...
                title: '測定可能な施策',
                description:
                  'Google Analytics、UTMパラメータで効果測定。データに基づく改善で成功率が3倍に',
              },
            ]
          );
        } else if (
          userInput.includes('ターゲット') ||
          userInput.includes('顧客') ||
          userInput.includes('ペルソナ')
        ) {
          addAriaMessage(
            ...
            [
              ...
              },
              {
                ...
                title: '1000人の熱狂ファン',
                description:
                  '100万人の認知より、1000人の熱狂的ファン。彼らがビジネスを支える基盤に',
              },
              {
                type: 'risk',
                title: 'ターゲット拡大の罠',
          ...
          userInput.includes('計画') ||
          userInput.includes('ロードマップ') ||
          userInput.includes('何から')
        ) {
          addAriaMessage(
          ...
          userInput.includes('不安') ||
          userInput.includes('心配') ||
          userInput.includes('失敗')
        ) {
          addAriaMessage(
            ...
            [
              {
                ...
                title: 'あなたに合った道',
                description:
                  '人それぞれのペース、スタイルがあります。あなたらしい方法を一緒に見つけましょう',
              },
              {
                type: 'opportunity',
                title: '対話の力',
                description:
                  '話すことで考えが整理される。言語化することで、次のステップが見えてくる',
              },
            ]
          );
        }
  ...
  const generateAnalysisFromConversation = () => {
    ...
    let strengths = ['明確なビジョンと情熱を持っている', '継続的な学習意欲'];
    let opportunities = ['デジタル化の波に乗るチャンス', 'ニッチ市場での先行者優位性'];
    let risks = ['競合の参入による市場の変化', '技術の急速な進歩への対応'];
    let recommendations = [
        ...
        datasets: [
          {
            ...
            data: analysisData.strengths.map((_: any, index: number) =>
              Math.round(
                (100 / analysisData.strengths.length) * (analysisData.strengths.length - index)
              )
            ),
            backgroundColor: [
              'rgba(54, 162, 235, 0.6)',
        ...
        datasets: [
          {
            ...
            data: analysisData.opportunities.map((_: any, index: number) =>
              Math.round(
                (100 / analysisData.opportunities.length) *
                  (analysisData.opportunities.length - index)
              )
            ),
      ...
      <div className="space-y-6">
        {parts.map((part, index) => {
          ...
          if (part === '[CHART:MARKET_INSIGHTS]') {
            ...
            return (
              <div key={index} className="my-4">
                <h4 className="text-sm font-semibold mb-3 text-blue-600 dark:text-blue-400 flex items-center">
                  <Target className="w-4 h-4 mr-2" />
                  💪 あなたの強み
                </h4>
                <div className="bg-blue-100/70 dark:bg-slate-800/90 rounded-xl p-4 border border-blue-300 dark:border-blue-400/30">
                  <div className="max-w-xs mx-auto">
                    <Doughnut
            ...
            return (
              <div key={index} className="my-4">
                <h4 className="text-sm font-semibold mb-3 text-green-600 dark:text-green-400 flex items-center">
                  <Lightbulb className="w-4 h-4 mr-2" />
                  🚀 チャンス
                </h4>
                <div className="bg-green-100/70 dark:bg-slate-800/90 rounded-xl p-4 border border-green-300 dark:border-green-400/30">
                  <Bar
            ...
            return (
              <div key={index} className="my-4">
                <h4 className="text-sm font-semibold mb-3 text-orange-600 dark:text-orange-400 flex items-center">
                  <Zap className="w-4 h-4 mr-2" />
                  🎯 推奨アクション
                </h4>
                <div className="bg-orange-100/70 dark:bg-slate-800/90 rounded-xl p-4 border border-orange-300 dark:border-orange-400/30">
                  <Bar
            ...
            return (
              <div key={index} className="my-4">
                <h4 className="text-sm font-semibold mb-3 text-purple-600 dark:text-purple-400 flex items-center">
                  <Brain className="w-4 h-4 mr-2" />
                  📊 SWOT総合分析
                </h4>
                <div className="bg-purple-100/70 dark:bg-slate-800/90 rounded-xl p-4 border border-purple-300 dark:border-purple-400/30">
            ...
            return (
              <p
                key={index}
                className="whitespace-pre-wrap leading-relaxed text-gray-800 dark:text-gray-100"
              >
                {part}
              </p>
            );
                  ...
                  {isAssistant && (
                    <div className="flex-shrink-0">
                      <div className="relative">
                        <Avatar
                          className={`w-10 h-10 ring-2 ring-gold/40 ${message.isStreaming ? 'animate-aria-pulse' : ''}`}
                        >
                      ...
                      <div className="mt-4 space-y-2">
                        {message.insights.map((insight, index) => (
                          ...
                          >
                            ...
                            >
                              {insight.type === 'strategy' && (
                                <Target className="w-3 h-3 text-aria-dark-soft" />
                              )}
                              {insight.type === 'opportunity' && (
                            ...
                            <div className="flex-1">
                              <h5 className="font-medium text-sm text-gray-800 dark:text-gray-100">
                                {insight.title}
                              </h5>
                ...
                <div className="gold-soft-tint dark:bg-gradient-to-br dark:from-slate-800 dark:to-slate-700 border border-gold/20 dark:border-gold/30 rounded-2xl p-4">
                  <div className="flex items-center space-x-2">
                    <div className="flex space-x-1">
                      ...
                      <div
                        className="w-2 h-2 gold-soft-gradient rounded-full animate-bounce"
        ...
        <CardContent className="p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Badge
              ...
              onClick={() =>
                !ariaThinking && !isStreaming && handleSuggestionClick('アイデアがあるんだけど...')
              }
            >
            ...
            <Badge
              variant="outline"
              className={`border-orange-400 dark:border-bronze/30 text-orange-700 dark:text-bronze bg-orange-100/60 dark:bg-slate-700/70 transition-all duration-300 ${!ariaThinking && !isStreaming ? 'cursor-pointer hover:bg-orange-200/60 dark:hover:bg-slate-600/80 hover:border-orange-500 dark:hover:border-bronze/50 hover:scale-105 hover:shadow-md hover:shadow-orange-400/20 dark:hover:shadow-bronze/20' : 'opacity-50 cursor-not-allowed'}`}
              onClick={() =>
                !ariaThinking && !isStreaming && handleSuggestionClick('競合が気になって')
              }
            >
            ...
            <Badge
              variant="outline"
              className={`border-yellow-400 dark:border-copper/30 text-yellow-700 dark:text-copper bg-yellow-100/60 dark:bg-slate-700/70 transition-all duration-300 ${!ariaThinking && !isStreaming ? 'cursor-pointer hover:bg-yellow-200/60 dark:hover:bg-slate-600/80 hover:border-yellow-500 dark:hover:border-copper/50 hover:scale-105 hover:shadow-md hover:shadow-yellow-400/20 dark:hover:shadow-copper/20' : 'opacity-50 cursor-not-allowed'}`}
              onClick={() =>
                !ariaThinking && !isStreaming && handleSuggestionClick('これから何をすればいいか')
              }
            >
