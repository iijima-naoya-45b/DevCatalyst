...
export class TokenManager {
  ...
  static setToken(token: string): void {
    if (typeof window !== 'undefined') {
      // アクセストークン: 1日（JWTトークンの有効期限に合わせる）
      CookieManager.setCookie(this.ACCESS_TOKEN_KEY, token, {
        days: 1,
        path: '/',
        sameSite: 'Lax',
      });
  ...
  static getAccessToken(): string | null {
    return (
      CookieManager.getCookie(this.ACCESS_TOKEN_KEY) || CookieManager.getCookie(this.TOKEN_KEY)
    );
  }
  ...
  static setUser(user: User): void {
    if (typeof window !== 'undefined') {
      CookieManager.setCookie(this.USER_KEY, JSON.stringify(user), {
        days: 7,
        path: '/',
        sameSite: 'Lax',
      });
