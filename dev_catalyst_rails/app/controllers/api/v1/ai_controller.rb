    ...
    class AiController < Api::V1::BaseController
      ...
      def chatStream
        ...
        begin
          ...
          Rails.logger.error "Api::V1::AiController#chatStream: #{e.class} - #{e.message}"
          errorPayload = {
            error: true,
      ...
      def processSseChunk(chatState, chunk)
        ...
        while (separator_index = chatState[:buffer].index("\n\n"))
          ...
          raw_event.each_line do |line|
            ...
            chatState[:assistant_content] << data["content"] if data["content"].present?

            chatState[:stream_completed] = true if data["done"]
          end
        end
      end
      ...
      def buildMetadataPayload(rawMetadata)
        ...
        Rails.logger.warn "Api::V1::AiController#buildMetadataPayload: Failed to normalize metadata. Error=#{e.class} Message=#{e.message}"
        {}
      end
    end
  end
