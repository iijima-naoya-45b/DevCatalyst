...
class PsychologicalProfile < ApplicationRecord
  ...
  validates :learning_style, inclusion: { 
    in: %w[visual auditory kinesthetic reading_writing multimodal],
  ...
  def add_bias_awareness(bias_type)
    ...
    self.bias_awareness ||= []
    unless bias_awareness.include?(bias_type.to_s)
  ...
  def add_motivation_factor(factor)
    ...
    unless motivation_factors.include?(factor.to_s)
  ...
  def learning_style_suggestions
    return [] unless learning_style.present?
