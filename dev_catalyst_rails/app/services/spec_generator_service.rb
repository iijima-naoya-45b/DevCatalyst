...
class SpecGeneratorService
  ...
  def call_fastapi_ai(prompt)
    ...
    raise "FastAPI error: #{response.status}" unless response.success?
  ...
  def build_section_prompt(_section_type, template, context)
    <<~PROMPT
  ...
  def determine_next_section_type(spec)
    ...
    optional_sections = SECTION_TEMPLATES.reject { |_, t| t[:required] }.keys
  ...
  # 最初の質問を生成
  def generate_initial_question(_section_type, template, spec)
  ...
  # フォローアップ質問を生成
  def generate_followup_question(_section_type, template, existing_content)
    prompt = <<~PROMPT
      既存の情報:
      #{existing_content}
  ...
  def update_section_from_response(spec, section_type, user_response)
    ...
    section.content = if section.content.present?
                        merge_section_content(section.content, new_content)
                      else
                        new_content
                      end
